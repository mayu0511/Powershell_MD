...
$DllPath = "C:\Users\mahendra.dwivedi\Desktop\KRA\clrjit.dll"
    ...
    try {
        ...
        if ($jsonContent -match '"AuthUrl"\s*:\s*"([^"]+)"') {
            ...
            if ($authUrl -match '^(https?:\/\/[^\/]+)') {
                $baseUrl = $Matches[1]
                try {
                    Invoke-WebRequest -Uri $baseUrl -Method GET -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop | Out-Null
                    $status = "PASSED"
                }
                catch {
