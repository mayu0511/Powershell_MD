######################################################################################################################
# Config Operation | DEVELOPED BY::Utsav Tyagi
# Version 4.3 | Updataion of NetworkManagementAPI in WCF and gmsa user Date: 22-July-2026 | Enhanced By Netra Chettri 
#======================================================================================================================
...
$JobsList = @("Stop IIS", "Start IIS", "Reset IIS", "Stop and Disable W3SVC", "Enable and Start W3SVC", "Stop ScaleService", "Start ScaleService", "Recycle AppPool", "Set Connect As - gmsa-app-svc$", "Set Connect As - gmsa-app-upg$", "Set Connect As - gmsa-web-svc$", "Set AppPool User - gmsa-app-svc$", "Set AppPool User - gmsa-app-upg$", "Set AppPool User - gmsa-web-svc$", "Set X-Request-ID", "Disable IIS Logging", "Enable IIS Logging", "Set Worker Process 1", "Set Worker Process 4", "Set Worker Process 64", "Enable32bitApplication True", "Enable32bitApplication False", "IdleTimeoutAction Terminate", "IdleTimeoutAction Suspend")
foreach($Jobs in $JobsList){$JobsComboBox.Items.Add($Jobs)}
#foreach($ProcessesTasks in $TasksList){$ProcessTaskComboBox.Items.Add($ProcessesTasks)}
#foreach($ServerTypes in $ServerTypesComboBoxList){$ServerTypesComboBox.Items.Add($ServerTypes)}
#foreach($AZs in $AZsComboBoxList){$AZsComboBox.Items.Add($AZs)}
...
$JobsList = @("Copy APP_SETUP", "Copy WCF_SETUP", "Copy WEB_SETUP", "Copy EWEB_SETUP", "Copy KMS_SETUP", "Copy RPD_SETUP")
foreach($Jobs in $JobsList){$JobsComboBox.Items.Add($Jobs)}
#foreach($ProcessesTasks in $TasksList){$ProcessTaskComboBox.Items.Add($ProcessesTasks)}
#foreach($ServerTypes in $ServerTypesComboBoxList){$ServerTypesComboBox.Items.Add($ServerTypes)}
#foreach($AZs in $AZsComboBoxList){$AZsComboBox.Items.Add($AZs)}
...
elseif(($JobsComboBox.SelectedItem.ToString() -eq "Recycle AppPool")  -or ($JobsComboBox.SelectedItem.ToString() -eq "Set Connect As - gmsa-app-svc$") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set Connect As - gmsa-app-upg$") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set Connect As - gmsa-web-svc$") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set AppPool User - gmsa-app-svc$") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set AppPool User - gmsa-app-upg$") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set AppPool User - gmsa-web-svc$") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set X-Request-ID") -or ($JobsComboBox.SelectedItem.ToString() -eq "Disable IIS Logging") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set Worker Process 1") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set Worker Process 4") -or ($JobsComboBox.SelectedItem.ToString() -eq "Set Worker Process 64")-or ($JobsComboBox.SelectedItem.ToString() -eq "Enable32bitApplication True")-or ($JobsComboBox.SelectedItem.ToString() -eq "Enable32bitApplication False")){
...
New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\WCF_SETUP\WebServer\NetworkManagementAPI\"
...
elseif(($JobsComboBox.SelectedItem.ToString() -eq "Copy RPD_SETUP")){
...
New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\RPD_SETUP\ReportDelivery\ReportDelivery\"
New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\RPD_SETUP\ReportDelivery\DataFeed\"
New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\RPD_SETUP\ReportDelivery\DataReports\"
New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\RPD_SETUP\CC_Python\"
New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\RPD_SETUP\PlatformCode\"

New-Item -ItemType Directory -Force -Path "$CopyFileDirectory\RPD_SETUP_Archive\"
...
If(($Site -eq "WCF") -or ($Site -eq "CoreCardServices") -or ($Site -eq "NetworkManagementAPI")){
...
If ($JobsParam -eq "Set Connect As - gmsa-app-svc$"){
...
If ($JobsParam -eq "Set Connect As - gmsa-app-upg$"){
$EnvironmentName = $EnvironmentName.ToUpper()
...
If ($JobsParam -eq "Set Connect As - gmsa-web-svc$"){
$EnvironmentName = $EnvironmentName.ToUpper()
...
If ($JobsParam -eq "Set AppPool User - gmsa-app-svc$"){
$EnvironmentName = $EnvironmentName.ToUpper()
$SecretObj = (Get-SECSecretValue -Secretid "$EnvironmentName/app-service-secret")
$Secret = $SecretObj.SecretString | ConvertFrom-Json
$DomainName = ($Secret.domainNetBIOSName).ToUpper()
...
If ($JobsParam -eq "Set AppPool User - gmsa-app-upg$"){
$EnvironmentName = $EnvironmentName.ToUpper()
$SecretObj = (Get-SECSecretValue -Secretid "$EnvironmentName/app-upgrade-secret")
$Secret = $SecretObj.SecretString | ConvertFrom-Json
$DomainName = ($Secret.domainNetBIOSName).ToUpper()
...
If ($JobsParam -eq "Set AppPool User - gmsa-web-svc$"){
$EnvironmentName = $EnvironmentName.ToUpper()
$SecretObj = (Get-SECSecretValue -Secretid "$EnvironmentName/web-service-secret")
$Secret = $SecretObj.SecretString | ConvertFrom-Json
$DomainName = ($Secret.domainNetBIOSName).ToUpper()
