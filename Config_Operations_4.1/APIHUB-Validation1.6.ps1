...
# Version 1.6 | PODRedirectService Site Access Update | Mahendra Dwivedi| Date:: 12-June-2026
#======================================================================================================================
...
if (Test-Path $jsonFile) {
    $fileContent = Get-Content -Path $jsonFile -Raw -Encoding UTF8
    $podMatches = [regex]::Matches($fileContent, '"(POD\d+)"\s*:\s*"([^"]+)"')

    foreach ($match in $podMatches) {
...
# ---------- PODRedirectService Site Access Validation ----------

Import-Module WebAdministration -ErrorAction Stop

$checkName = "PODRedirectService Site Access"
$value     = "Not Checked"
$status    = "FAILED"

$location = "PODRedirectService"
...
try {
    if (-not (Test-Path "IIS:\Sites\$location")) {
        ...
        $bindings = (Get-IISSite $location).Bindings
        $httpsBinding = $bindings | Where-Object { $_.protocol -eq "https" }

        if (-not $httpsBinding) {
            $value  = "No HTTPS Binding"
            $status = "FAILED"
        }
        else {
            $ipAddress = $httpsBinding.bindingInformation.Split(':')[0]

            if ($ipAddress -eq "*" -or [string]::IsNullOrWhiteSpace($ipAddress)) {
                $ipAddress = "localhost"
            }

            $siteUrl = "https://$ipAddress/Index.html"
            $value   = $siteUrl

            try {
                $response = Invoke-WebRequest -Uri $siteUrl -TimeoutSec 120 -UseBasicParsing -ErrorAction Stop
