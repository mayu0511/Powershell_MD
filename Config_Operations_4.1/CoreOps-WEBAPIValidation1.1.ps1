...
$ExpectedWebApiUrlWorking = "Working"
$ExpectedWebApiUrlAngularWorking = "Working"
...
$Outputreport = "<HTML><TITLE> $Module Validation Report </TITLE>
                 ...
                 <TR bgcolor=gray align=center>
				   ...
				   <TD><B>WebApiUrl Status</B></TD>
                   <TD><B>WebApiUrlAngular Status</B></TD>
...
$Outputreport += "<TD>$("$MasterHost".ToUpper())</TD>
				   ...
				   <TD>$("$ExpectedWebApiUrlWorking")</TD>
				   <TD>$("$ExpectedWebApiUrlAngularWorking")</TD>
...
$filePath = "D:\WebServer\CoreOps\appsettings.json"

$WebApiUrlStatus = "Not Working"
$WebApiUrlAngularStatus = "Not Working"
...
if (!(Test-Path $filePath)) {
    ...
    $urls = @(
        @{ Name = "WebApiUrl"; Url = $json.AppSettings.WebApiUrl },
        @{ Name = "WebApiUrlAngular"; Url = $json.AppSettings.WebApiUrlAngular }
    )
    ...
    foreach ($item in $urls) {
        ...
        try {
            $CurrentRequest = Invoke-WebRequest -Uri $CurrentURL -UseBasicParsing
            $CurrentStatus = $CurrentRequest.StatusCode
        }
        catch {
            if ($_.Exception.Response) {
                $CurrentStatus = [int]$_.Exception.Response.StatusCode
            }
        }

        if ($CurrentStatus -eq 404) {
            if ($item.Name -eq "WebApiUrl") {
                $WebApiUrlStatus = "Working"
            }
            else {
                $WebApiUrlAngularStatus = "Working"
            }
        }
        else {
            if ($item.Name -eq "WebApiUrl") {
...
-and ($WebApiUrlStatus -eq $ExpectedWebApiUrlWorking) `
-and ($WebApiUrlAngularStatus -eq $ExpectedWebApiUrlAngularWorking))
{
$Outputreport += "<TR align=center>" 
}else{
...
$Outputreport += "<TD>$($Entry.ServerName)</TD>
				  ...
				  <TD align=center>$WebApiUrlStatus</TD>
                  <TD align=center>$WebApiUrlAngularStatus</TD>
...
Clear-Variable WebApiUrlStatus
Clear-Variable WebApiUrlAngularStatus
