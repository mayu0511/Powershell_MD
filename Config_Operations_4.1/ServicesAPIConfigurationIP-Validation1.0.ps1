############################################################################################################
#API Configuration Validation | DEVELOPED BY:: Mahendra Dwivedi
#Version 1.2 API Configuration Validation| Updated by Mahendra | 05-May-26
############################################################################################################
...
function Test-Url {
    ...
    try {
        ...
        if ($c -notmatch "virtloc=$api") {
            return "FAILED - virtloc mismatch"
        }

        # ? FIXED CONDITION (OR logic)
        if ($c -notmatch "appName=appServices" -and $c -notmatch "appName=appCoreAuth" -and $c -notmatch "appName=appCoreAcquire") {
            return "FAILED - appName missing"
        }

        if ($c -notmatch "serverPort=") {
            return "FAILED - serverPort missing"
        }
