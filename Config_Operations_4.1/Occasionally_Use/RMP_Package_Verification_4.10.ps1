...
# Version 4.9 | Add Intent file method if intent if not there final zip will not happen 
# Version 4.10 | Added Release Notes RD CreditCloudStatementing .txt file is present then final zip will not happen and Remove the CoreCredit folder from Webserver | Netra Chettri | Date:: 11 Aug 2026
#========================================================================================================================================================================================================
...
function Compress-With7Zip {
    ...
    # âœ… Include the root folder (by zipping the folder name from its parent)
...
# ------------------ Step 6: List ZIP Contents ------------------
    ...
    $deleteList = @(
        ...
        "\Consolidated_Package\WebServer\CoreCredit",
    ...
    # âœ… Print all expected items and their count
    ...
    try {
        ...
        # âœ… Pass arguments as an array to avoid quoting problems
        $arguments = @(
            "a",                     # Add to archive
            "-tzip",                 # Format: ZIP
...
if ($confirmDeleteFolders -match '^[Yy]$') {
    ...
    # ------------------ Step 11: List All Files and Folders in Package Root ------------------

$intentFileAvailable = $false
$releaseNotesAvailable = $false

$confirmFinalCheck = Read-Host "Step 11: Do you want to List All Files and Folders in Package Root? (Y/N)"
...
if ($confirmFinalCheck -match '^[Yy]$') {
    ...
    if (Test-Path $sourceFolder) {
        ...
        if ($allItems.Count -eq 0) {
            ...
            # ---------------------------------------------------------
            # Check for Intent Files ZIP in Package Root
            # ---------------------------------------------------------

            $intentFiles = Get-ChildItem -Path $sourceFolder -Filter "*_IntentFiles.zip" -File -ErrorAction SilentlyContinue

            if ($intentFiles.Count -gt 0) {

                $intentFileAvailable = $true

                Write-Host ""
                Write-Host "Intent File Found:" -ForegroundColor Green

                foreach ($intentFile in $intentFiles) {
                    Write-Host "  $($intentFile.Name)" -ForegroundColor Green
                    Log-Step "Intent file found: $($intentFile.FullName)"
                }
                ...
                Write-Host ""
                Write-Host "Intent file is NOT available in the package root folder." -ForegroundColor Red
                Log-Step "Intent file *_IntentFiles.zip NOT found in package root: $sourceFolder"
                ...
                [System.Windows.MessageBox]::Show(
                    "Intent file is not available in the package root folder.`n`nPlease keep/copy the *_IntentFiles.zip file into the package root folder.`n`nStep 12 will NOT be executed until the Intent File is available.",
            ...
            # ---------------------------------------------------------
            # Check for Release Notes TXT in Package Root
            # ---------------------------------------------------------

            $releaseNotesFiles = Get-ChildItem -Path $sourceFolder -Filter "Release Notes RD CreditCloudStatementing-*.txt" -File -ErrorAction SilentlyContinue

            if ($releaseNotesFiles.Count -gt 0) {

                $releaseNotesAvailable = $true

                Write-Host ""
                Write-Host "Release Notes File Found:" -ForegroundColor Green

                foreach ($releaseNotesFile in $releaseNotesFiles) {
                    Write-Host "  $($releaseNotesFile.Name)" -ForegroundColor Green
                    Log-Step "Release Notes file found: $($releaseNotesFile.FullName)"
                }
                ...
                Write-Host ""
                Write-Host "Release Notes file is NOT available in the package root folder." -ForegroundColor Red
                Log-Step "Release Notes file Release Notes RD CreditCloudStatementing-*.txt NOT found in package root: $sourceFolder"
                ...
                [System.Windows.MessageBox]::Show(
                    "Release Notes file is not available in the package root folder.`n`nPlease keep/copy the Release Notes RD CreditCloudStatementing-*.txt file into the package root folder.`n`nStep 12 will NOT be executed until the Release Notes file is available.",
        ...
        [System.Windows.MessageBox]::Show(
            "Package root folder was not found:`n`n$sourceFolder`n`nStep 12 will NOT be executed.",
    ...
    Write-Host "Skipped listing of files and folders in package root folder." -ForegroundColor Yellow
    Log-Step "Skipped listing of items in $sourceFolder"

    Write-Host ""
    Write-Host "Step 12 will NOT be executed because Step 11 was skipped." -ForegroundColor Yellow
}


# ------------------ Step 12: Zip the Main Package Folder (Faster using 7-Zip) ------------------

if ($intentFileAvailable -and $releaseNotesAvailable) {

    Write-Host ""
    Write-Host "Intent File and Release Notes are available. Proceeding with Step 12..."  -ForegroundColor Green
    Log-Step "Intent file available. Proceeding with Step 12."

    # ---------------------------------------------------------
    # Function: Create Fast ZIP using 7-Zip
    # ---------------------------------------------------------
    ...
    function Create-FastZipWith7Zip {
        ...
        # Includes the root package folder
        ...
        try {
            ...
            Log-Step "Starting 7-Zip compression."
            Log-Step "Source folder: $SourceFolder"
            Log-Step "ZIP path: $ZipPath"
            ...
            if ($process.ExitCode -eq 0) {
                ...
                if (Test-Path $ZipPath) {
                    ...
                    Write-Host "7-Zip completed, but ZIP file was not found." -ForegroundColor Red
                    Log-Step "7-Zip completed but ZIP file was not found: $ZipPath"
    ...
    if ($confirmManualZip -match '^[Yy]$') {
        ...
        $manualSourceFolder = $sourceFolder
        $manualZipFile      = "$manualSourceFolder.zip"
        ...
        if (Test-Path $manualSourceFolder) {
            ...
            try {
                ...
                $zipCreated = Create-FastZipWith7Zip `
                    -SourceFolder $manualSourceFolder `
                    -ZipPath $manualZipFile


                # -------------------------------------------------
                # Validate ZIP only if creation succeeded
                # -------------------------------------------------

                if ($zipCreated -and (Test-Path $manualZipFile)) {

                    try {

                        Add-Type -AssemblyName System.IO.Compression.FileSystem

                        $zipArchive = [System.IO.Compression.ZipFile]::OpenRead($manualZipFile)

                        $zipArchive.Dispose()

                        Write-Host ""
                        Write-Host "ZIP file is valid and can be opened." -ForegroundColor Green
                        Write-Host "ZIP File: $manualZipFile" -ForegroundColor Green
                    ...
                    Write-Host ""
                    Write-Host "ZIP creation failed. ZIP validation skipped." -ForegroundColor Red

                    Log-Step "ZIP creation failed. ZIP validation skipped."
                }
    ...
    Log-Step "Step 12 completed."
    Write-Host ""
    Write-Host "Step 12 completed." -ForegroundColor Green

}
else {

    # ---------------------------------------------------------
    # Intent File Missing - Step 12 MUST NOT RUN
    # ---------------------------------------------------------

    Write-Host ""
    Write-Host "====================================================" -ForegroundColor Red
    Write-Host "Step 12 SKIPPED" -ForegroundColor Red
    Write-Host "====================================================" -ForegroundColor Red

    Write-Host ""
    if (-not $intentFileAvailable -and -not $releaseNotesAvailable) {
        Write-Host "Reason: *_IntentFiles.zip and Release Notes RD CreditCloudStatementing-*.txt are not available in the package root folder." -ForegroundColor Yellow
        Write-Host "Please keep/copy both files into the package root folder." -ForegroundColor Yellow
        Log-Step "Step 12 SKIPPED because both Intent File and Release Notes were not found in package root."
    }
    elseif (-not $intentFileAvailable) {
        Write-Host "Reason: *_IntentFiles.zip is not available in the package root folder." -ForegroundColor Yellow
        Write-Host "Please keep/copy the Intent File into the package root folder." -ForegroundColor Yellow
        Log-Step "Step 12 SKIPPED because *_IntentFiles.zip was not found in package root."
    }
    else {
        Write-Host "Reason: Release Notes RD CreditCloudStatementing-*.txt is not available in the package root folder." -ForegroundColor Yellow
        Write-Host "Please keep/copy the Release Notes file into the package root folder." -ForegroundColor Yellow
        Log-Step "Step 12 SKIPPED because Release Notes file was not found in package root."
    }
    Write-Host ""
...
if ($zipConfirm -match '^[Yy]$') {
    if (Test-Path $sourceFolder) {
        ...
        try {
    # âœ… Create ZIP with 7-Zip including the root folder
...
if (Test-Path $consFolder) {
    ...
    # âœ… Always log and show Step 6 as skipped (even if Y was selected)
