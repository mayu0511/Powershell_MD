...
#------------Clear All Variable------
    ...
    $packagedir = 'E:\Packages\COOKIE\POD2\PATQA\'
    ...
    foreach ($folder in $foldersToDelete) {
        if (Test-Path $folder) {
            try {
                ...
                Write-Host "🗑️ Deleted folder: $folder" -ForegroundColor Yellow
            ...
            Write-Host "⚠️ Folder not found: $folder" -ForegroundColor DarkYellow
            Log-Step "Folder not found: $folder"
        }
    }
} else {
    Write-Host "⏭️ Skipping folder deletion..." -ForegroundColor Yellow
...
function Compress-With7Zip {
    ...
    try {
        ...
        if ($process.ExitCode -eq 0) {
            Write-Host "`n✅ ZIP created (includes root folder): $destinationZip" -ForegroundColor Green
            Log-Step "ZIP created (includes root folder): $destinationZip"
        } else {
            Write-Host "`n❌ 7-Zip failed with exit code $($process.ExitCode)" -ForegroundColor Red
            Log-Step "7-Zip failed with exit code $($process.ExitCode)"
        }
    } catch {
        Write-Host "`n❌ Exception during 7-Zip compression: $($_.Exception.Message)" -ForegroundColor Red
...
# ------------------ Prompt and run Step 8 ------------------
    ...
    $confirmZipFinal = Read-Host "`nStep 8: Do you want to ZIP the final 'Package' folder into 'Package.zip'? (Y/N)"
...
$confirmDeleteFolders = Read-Host "`nStep 9: Do you want to delete both 'Consolidated_Package' and 'Package' folders? (Y/N)"
...
if ($confirmDeleteFolders -match '^[Yy]$') {
    ...
    $confirmFinalCheck = Read-Host "`nStep 10: Do you want to List All Files and Folders in Package Root? (Y/N)"
    ...
    $confirmChecksum = Read-Host "`nStep 11: Do you want to generate checksum of Package.zip? (Y/N)"
...
function Create-FastZipWith7Zip {
    ...
    try {
        ...
        Write-Host "`n❌ Exception during ZIP: $($_.Exception.Message)" -ForegroundColor Red
...
$confirmManualZip = Read-Host "`nStep 12: Do you want to ZIP the full '$packagename' folder now? (Y/N)"
...
Write-Host "`n🔍 Checking first-level subfolders of Consolidated_Package..." -ForegroundColor Cyan
$confirmZipEach = Read-Host "`nStep 5: Do you want to Check Blank Folders and Delete'? (Y/N)"
...
if (Test-Path $consFolder) {
    ...
    if ($firstLevelFolders.Count -gt 0) {
        foreach ($folder in $firstLevelFolders) {
            ...
            if ($items.Count -eq 0) {
                $confirmDel = Read-Host "`n➡️ Folder '$($folder.Name)' is empty. Do you want to delete it? (Y/N)"
                if ($confirmDel -match '^[Yy]$') {
                    try {
                    ...
                    Write-Host "⏭️ Skipped empty folder: $($folder.FullName)" -ForegroundColor Yellow
    ...
    $confirmZipEach = Read-Host "`nStep 6: Do you want to ZIP each folder inside 'Consolidated_Package' to 'Package'? (Y/N)"
    if ($confirmZipEach -match '^[Yy]$') {
    $consolidatedPath = Join-Path $sourceFolder "Consolidated_Package"
    $packagePath      = Join-Path $sourceFolder "Package"
    ...
    Write-Host "⏭️ Skipped Step 7 - Validate Items in Package." -ForegroundColor Yellow
