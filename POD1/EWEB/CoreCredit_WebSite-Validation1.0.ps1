...
# Version| Updated by Date:: 7-July-2026
#======================================================================================================================
...
$LetterServerURL = $Entry.LetterServerPath
...
# ============================
# Report & Letter Server Check
# ============================

# Force TLS 1.2 (important for PROD)
...
function Test-WebURL {
    ...
    Write-Output "`nChecking URL: $Url"
    ...
    try {
        ...
        Write-Output "ERROR while calling $Url"
...
try {
    ...
    Write-Output "Credential not loaded, will try without credential"
}

# ============================
# Use ACTUAL URLs from config
# ============================

$ReportServerURL = $Entry.ReportServerPath
$LetterServerURL = $Entry.LetterServerPath

# Remove trailing slash (safe cleanup)
$ReportServerURL = $ReportServerURL.TrimEnd('/')
$LetterServerURL = $LetterServerURL.TrimEnd('/')

# ============================
# Execute Checks
# ============================

# Try WITH credential first, fallback to WITHOUT
$ReportServerURLSTATUS = Test-WebURL -Url $ReportServerURL #-Credential $cred

if ($ReportServerURLSTATUS -eq "Failure" -and $cred) {
    Write-Output "Retrying WITHOUT credential..."
    $ReportServerURLSTATUS = Test-WebURL -Url $ReportServerURL
}

$LetterServerURLSTATUS = Test-WebURL -Url $LetterServerURL # -Credential $cred

if ($LetterServerURLSTATUS -eq "Failure" -and $cred) {
    Write-Output "Retrying WITHOUT credential..."
    $LetterServerURLSTATUS = Test-WebURL -Url $LetterServerURL
}

Write-Output "Final Report Server Status: $ReportServerURLSTATUS"
Write-Output "Final Letter Server Status: $LetterServerURLSTATUS"
}
Catch{
# Error caught but not needed for processing
}
...
$Outputreport += "<TD>$($Entry.ServerName)</TD>
      ...
      <TD align=center>$($Entry.LetterServerPath) $LetterServerURLSTATUS </TD>
