######################################################################################################################
# User Chnage for WCF App Pool | DEVELOPED BY:: Mahendra Dwivedi
# Version 1.0 | Initial Release | Date:: 24-April-2025
#=====================================================================================================================
...
if ($Environmentattributon -eq "cookie") {
    Clear-Host
    Write-Host "1. COOKIE - POD1"
    Write-Host "2. COOKIE - POD2"
    Write-Host "3. COOKIE - POD3 (POD4)"
    
    $PODNumberSelected = Read-Host "Enter your choice (1 or 2 or 3)"
    
    if ($PODNumberSelected -eq "1") {
        $PODName = "pod1"
     }elseif ($PODNumberSelected -eq "2") {
        $PODName = "pod2"
    } elseif ($PODNumberSelected -eq "3") {
        $PODName = "pod4"
    } else {
        $PODName = $NULL
    ...
    # PODName is set above based on user selection; do not override here
...
$PODSegment = if ($PODName) { $PODName } else { "" }
...
ForEach ($ServerType in $ServerTypeList) {
    ...
    $nameFilter = "*$ServerType$ShortRegion$EnvironmentName$EnvironmentStack$PODSegment*"
    $AvailabilityFilter = "$AvailabilityZone"

    $ServerList += ((aws ec2 describe-instances --query "Reservations[*].Instances[*].{AvailabilityZone:Placement.AvailabilityZone,IpAddress:PrivateIpAddress,Type:InstanceType,Name:Tags[?Key=='Name']|[0].Value,Status:State.Name}" --filters "Name=instance-state-name,Values=running" "Name=tag:Name,Values='$nameFilter'" "Name=availability-zone,Values='$AvailabilityFilter'" --region $Region | ConvertFrom-Json) | Select-Object @{n = "Name"; e = { $_.Name } }, @{n = "AvailabilityZone"; e = { $_.AvailabilityZone } }, @{n = "serial"; e = { [double]($_.Name -split "$EnvironmentName$EnvironmentStack")[1] } } | Sort-Object -Property serial)
}
...
foreach ($computername in $ServerList) {
    try {
        $result = Invoke-Command -ComputerName $computername -SessionOption $option -ErrorAction Stop -ScriptBlock {
            ...
            $userName = "CC-POD2-PROD\gmsa-app-svc$"
            $password = ""
