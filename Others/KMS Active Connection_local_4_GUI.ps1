...
$txtFile.Text = "E:\CoreCard\KMS\Service\Data\kms-trace2025-09-16.txt"
...
$txtKeywords.Text = "Error Code,decrypt failed,Service Stop"
...
$btnRun.Add_Click({
    ...
    if (-not (Test-Path $ServerListFile)) {
        [System.Windows.Forms.MessageBox]::Show("Server list file not found: $ServerListFile","Error","OK","Error")
        return
    }
    ...
    foreach ($computername in $ServerList) {
        ...
        if ($logLines.Count -eq 0 -or $logLines -contains "NO_LOG_FILE_FOUND") {
            ...
            foreach ($line in $logLines) {
                $statusClass = "ok"
                if ($keywords -match "Error Code|decrypt failed|Service Stop") {
