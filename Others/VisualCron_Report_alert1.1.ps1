...
# Extract job info with trigger time for sorting
$jobList = foreach ($job in $jobslistxml.ArrayOfJobClass.JobClass) {
    $triggerTime = $job.Triggers.TriggerClass.Description
    if (-not $triggerTime) { $triggerTime = "9999- No Trigger Description" } # put no trigger at bottom
    [PSCustomObject]@{
        Job       = $job
        JobName   = $job.Name
        Trigger   = $triggerTime
...
# Sort by trigger time string
$jobList = $jobList | Sort-Object Trigger
