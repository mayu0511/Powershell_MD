...
# Expected Task Names and Working Directories
$TaskMappings = @(
...
foreach ($svc in $ServiceMappings) {
    ...
    if ($service) {
        ...
        # Compare executable path (strip quotes if present)
        $actualPath = $service.PathName.Trim('"')
        if ($actualPath -like "$($svc.Path)*") {
            $actualPath = "<span style='color:green;'>$actualPath</span>"
        } else {
            $actualPath = "<span style='color:red;'>$actualPath</span>"
        }
