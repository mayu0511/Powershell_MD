...
foreach ($Server in $ServerList) {
    ...
    $result = Invoke-Command -ComputerName $Server -SessionOption $option -ScriptBlock {
		...
		#------------------- IPMJsonGenerator Check
		$service = Get-Service | Where-Object { $_.Name -like "*IPMJson*" }

		if ($service) {
		if ($service.Status -eq "Running") {
        $output.IPMJsonGenerator = "Installed and Running"
		}
		elseif ($service.Status -eq "Stopped") {
        $output.IPMJsonGenerator = "Installed but Stopped"
		}
		else {
        $output.IPMJsonGenerator = "Installed - Status: $($service.Status)"
		}
		}
		else {
		$output.IPMJsonGenerator = "Not Required"
		}
...
$rows = foreach ($row in $ValidationResults) {
	...
	"<td>$(Format-Cell $row.IPMJsonGenerator)</td>" +
