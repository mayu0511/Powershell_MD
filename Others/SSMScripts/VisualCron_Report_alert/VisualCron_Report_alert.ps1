...
# ---------------- Sort Jobs by Name (A-Z) ----------------
$jobsSorted = $jobslistxml.ArrayOfJobClass.JobClass | Sort-Object { $_.Name.ToLower() }
...
<h2>VisualCron Job and Trigger Report</h2>
...
foreach ($job in $jobsSorted) {
    $sn = $sno++
    $jobName = $job.Name
    $jobActive = $job.Stats.Active

    # Collect and clean multiple trigger times
    $triggerTimes = @()
    foreach ($trigger in $job.Triggers.TriggerClass) {
        if ($trigger.Description) {
            $cleanTime = $trigger.Description -replace '\s+since\s+.*$', '' `
                                               -replace '^[Ee]very\s+year,?\s+[Ee]very\s+month,?\s*', '' `
                                               -replace 'Every day', 'Everyday'
            $triggerTimes += $cleanTime
        }
    }

    if ($triggerTimes.Count -gt 0) {
        # Sort chronologically where possible
        $triggerTimes = $triggerTimes | Sort-Object {
            $t = $_ -replace '^Everyday at ', ''
            try { [datetime]::ParseExact($t, 'h:mm:ss tt', $null) } catch { $_ }
        }
        $triggerTime = ($triggerTimes -join '<br/>')
    } else {
        $triggerTime = "No Trigger Description"
    }
...
# ---------------- Save HTML ----------------
$ReportPath = Join-Path $OutputFolder ("VCJOBTrigertime-{0}.html" -f (Get-Date -Format "MM-dd-yyyy-HH-mm-ss"))
