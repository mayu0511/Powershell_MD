...
# ---------------------------HTML Status ----
    ...
    $output.GPA = "<span style='color:green;'>Installed (Version: $($registryMatch.DisplayVersion))</span>"
    }
    elseif (-not $exeExists) {
    $output.GPA = "<span style='color:red;'>Not Installed - exe not available at $exePath</span>"
    }
    elseif (-not $registryMatch) {
    $output.GPA = "<span style='color:red;'>Not present in Registry and Not Installed</span>"
    }

    
#------------------- MonitoringScript Folder Check (D:\DBBSetup\MonitoringScript)
