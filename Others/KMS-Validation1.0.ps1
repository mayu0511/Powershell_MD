...
$DATABASEHOST = ([xml](Get-Content 'D:\KMS\KMM\KMService.exe.config')).configuration.appSettings.add | Where-Object { $_.key -eq 'CONNECTION_STRING' } | ForEach-Object { $_.value -replace '.*DSN=([^;]+).*','$1' }
