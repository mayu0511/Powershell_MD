...
# Version 1.3 | FileSplitter Validation | Date:: 07-Aug-2025
#======================================================================================================================
Clear-Host
...
# ----------------- Scheduled Task Details Check -----------------
$TaskNames = @(
    "ACHDownloaderSplitterService",
    "BillPayPaymentDownloaderSplitterService",
    "EmbossingDownloaderSplitterService",
    "LockBoxDownloaderSplitterService",
...
foreach ($taskName in $TaskNames) {
    ...
    if ($task) {
        ...
        $triggerTime = if ($trigger) { $trigger.StartBoundary } else { "N/A" }
...
# ----------------- FileSplitter Service Details Check -----------------
$ServiceNames = @(
    "PlatAmortizationFileSplitterService",
    "PlatChargebackFileSplitterService",
    "PlatEnrollmentFileSplitterService",
    "PlatIPMFileSplitterService",
    "PlatT140FileSplitter"
)
...
# ----------------- Simulated AllInstances for Environment Detection -----------------
# Replace below with actual instance data loading
$AllInstances = @(
    [PSCustomObject]@{ Name = 'ccfse1patuat1'; Environment = $null },
    [PSCustomObject]@{ Name = 'ccfse1prod1'; Environment = $null }
)

# ----------------- Detect Environments from Instance Names -----------------
$AllEnvironments = $AllInstances | ForEach-Object {
    if (-not $_.Environment -and ($_.Name -match 'patuat' -or $_.Name -match 'prod')) {
        if ($_.Name -match 'patuat') {
            $_.Environment = 'patuat'
        } elseif ($_.Name -match 'prod') {
            $_.Environment = 'PROD'
        }
    }
    $_
} | Select-Object -ExpandProperty Environment | Where-Object { $_ } | Sort-Object -Unique

# ----------------- AWS Secret Validation Function -----------------
function Get-SecretValue($SecretId) {
    try {
        $secret = Get-SECSecretValue -SecretId $SecretId -ErrorAction Stop
        return -not [string]::IsNullOrWhiteSpace($secret.SecretString)
    } catch {
        return $false
    }
}
...
# (Load your $Utilities hash table as you already have it — trimmed here for brevity)
# Example:
$Utilities = @{
    'PROD' = @(
...
# ----------------- Validate Secrets per Environment -----------------
foreach ($env in $AllEnvironments) {
    if ($env -in @('patuat', 'PROD')) {
        $DeduplicatedSecrets = @{}

        foreach ($utility in $Utilities.GetEnumerator() | Where-Object { $_.Key -eq $env }) {
            foreach ($secret in $utility.Value) {
                $compositeKey = "$($secret.VariableName.ToLower())|$($secret.KeyName.ToLower())"
                if (-not $DeduplicatedSecrets.ContainsKey($compositeKey)) {
                    $DeduplicatedSecrets[$compositeKey] = @{
        ...
        $envOrder = @{ $env = 1 }
        $unsortedResults = foreach ($secret in $DeduplicatedSecrets.Values) {
            $isValid = Get-SecretValue -SecretId $secret.KeyName
            [PSCustomObject]@{
                UtilityName  = $secret.UtilityName
                VariableName = $secret.VariableName
                KeyName      = $secret.KeyName
        ...
        $results = $unsortedResults | Sort-Object { $envOrder[$_.UtilityName] -as [int] -or 99 }, VariableName
        $results | Format-Table -AutoSize
        $resultsSecretValidation += $results
    }
}
