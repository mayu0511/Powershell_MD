    ...
    try {
        $DeletionResult = Invoke-Command -ComputerName $server -SessionOption $option -ScriptBlock {
            ...
            if (Test-Path $FolderPath) {
                Get-ChildItem -Path $FolderPath -Recurse -Force -ErrorAction SilentlyContinue | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue
                $filesLeft = Get-ChildItem -Path $FolderPath -Recurse -Force -ErrorAction SilentlyContinue
        ...
        } -ArgumentList "D:\BKP\DD\*" -ErrorAction Stop
