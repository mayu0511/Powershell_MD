...
# ---------------- Sort Jobs by Trigger Time ----------------
$jobsSorted = $jobslistxml.ArrayOfJobClass.JobClass | Sort-Object {
    $_.Triggers.TriggerClass.Description -replace '\s+since\s+.*$', '' `
...
foreach ($job in $jobsSorted) {
    ...
    if (-not $triggerTime) { 
        ...
        $triggerTime = $triggerTime -replace '\s+since\s+.*$', '' `
                                       -replace '^[Ee]very\s+year,?\s+[Ee]very\s+month,?\s*', ''
    }
