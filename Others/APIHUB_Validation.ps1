...
<style>
    ...
    .status-error { background-color: #f5c6cb; color: #721c24; font-weight: bold; }
</style>
"@

if ($results | Where-Object { $_.Status -match 'FAILED|Missing|ERROR|Not Found|Unable|Failed' }) {
    $summaryStatus = "<h3 style='color: red;'>Some checks failed. Please review the details below.</h3>"
} else {
    $summaryStatus = "<h3 style='color: green;'>All checks PASSED successfully.</h3>"
}

$htmlRows = foreach ($r in $results) {
    $statusClass = if ($r.Status -match "PASSED|AVAILABLE") {
        "status-PASSED"
    } elseif ($r.Status -match "FAILED|MISSING|ERROR|NOT FOUND|UNABLE") {
        "status-failed"
    } elseif ($r.Status -match "WARNING") {
        "status-warning"
    } else {
        ""
    }
