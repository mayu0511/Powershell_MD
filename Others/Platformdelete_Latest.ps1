...
Write-Host "1. Delete CC-Runtime"
Write-Host "2. Delete a shmem Memory file"
Write-Host "3. Delete a SpecificFile folder"
...
foreach ($server in $ServerList) {
    try {
        $DeletionResult = Invoke-Command -ComputerName $server -SessionOption $option -ScriptBlock {
            ...
            switch ($Condition) {
                1 {
                    # Condition 1: Delete all files inside a folder
                ...
                2 {
                    # Condition 2: Delete a specific file inside a folder
                    Remove-Item -Path $SpecificFilePath -Force
        ...
        } -ArgumentList $ConditionSelected, "D:\BKP\DD\*", "D:\BKP\2\rundbb.exe", "D:\BKP\1" -ErrorAction Stop
