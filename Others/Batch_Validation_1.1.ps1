...
foreach ($Server in $ServerList) {
    ...
    $result = Invoke-Command -ComputerName $Server -SessionOption $option -ScriptBlock {
        ...
        if (Test-Path $folderPath) {
            $folderSizeBytes = (Get-ChildItem -Path $folderPath -Recurse -Force -ErrorAction SilentlyContinue | 
                                Measure-Object -Property Length -Sum).Sum
            $folderSizeGB = [math]::Round($folderSizeBytes / 1GB, 2)
            $output.MonitoringScript = "$folderSizeGB GB"
        } else {
...
# ----------- Step 4: Generate HTML Report with Colored Rows -----------
...
<style>
    ...
    .healthy { background-color: #e0ffe0; }  /* green */
    .unhealthy { background-color: #ffe0e0; } /* red */
</style>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (text.includes('not installed') ||
            ...
            text.includes('error') ||
            text.includes('service not')) {
            row.classList.add('unhealthy');
        } else {
            row.classList.add('healthy');
        }
    }
});
</script>
...
$ValidationResults | ConvertTo-Html -Property ServerName, BCP, Python, PlatformVersion, TranslateExe, VisualCronService, GPA, MonitoringScript -Head $style -Title "Remote Server Validation Report" |
