...
function Create-ZipWithProgress {
    param (
        [string]$SourceFolder,
        [string]$ZipPath
    )
    ...
    # Copy the root folder into tempDir
    $folderName = Split-Path -Path $SourceFolder -Leaf
    $tempSource = Join-Path $tempDir $folderName
    Copy-Item -Path $SourceFolder -Destination $tempSource -Recurse

    try {
        [System.IO.Compression.ZipFile]::CreateFromDirectory($tempSource, $ZipPath, [System.IO.Compression.CompressionLevel]::Optimal, $true)

        foreach ($file in $files) {
            $progress++
            Write-Progress -Activity "📦 Zipping files" -Status "$progress of $total files" -PercentComplete (($progress / $total) * 100)
        }
...
if ($confirmManualZip -match '^[Yy]$') {
    ...
    if (Test-Path $manualSourceFolder) {
        try {
            ...
            # ✅ Use custom function to ZIP with root folder and progress
            Create-ZipWithProgress -SourceFolder $manualSourceFolder -ZipPath $manualZipFile
