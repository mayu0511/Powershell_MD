...
# Reference file paths and hashes (local)
...
# Generate hashes
$referenceHash1 = Get-FileHash -Path $referenceFile1 -Algorithm SHA256
$referenceHash2 = Get-FileHash -Path $referenceFile2 -Algorithm SHA256
$referenceHash3 = Get-FileHash -Path $referenceFile3 -Algorithm SHA256
$referenceHash4 = Get-FileHash -Path $referenceFile4 -Algorithm SHA256
...
foreach ($server in $ServerList) {
    ...
    try {
        ...
        $hashStatus1 = if ($remoteHashes.H1 -eq "NotFound") { "File Not Found" } elseif ($remoteHashes.H1 -eq $referenceHash1.Hash) { "Identical" } else { "Different" }
        $hashStatus2 = if ($remoteHashes.H2 -eq "NotFound") { "File Not Found" } elseif ($remoteHashes.H2 -eq $referenceHash2.Hash) { "Identical" } else { "Different" }
        $hashStatus3 = if ($remoteHashes.H3 -eq "NotFound") { "File Not Found" } elseif ($remoteHashes.H3 -eq $referenceHash3.Hash) { "Identical" } else { "Different" }
        $hashStatus4 = if ($remoteHashes.H4 -eq "NotFound") { "File Not Found" } elseif ($remoteHashes.H4 -eq $referenceHash4.Hash) { "Identical" } else { "Different" }

    } catch {
        $hashStatus1 = "Connection Failed"
        $hashStatus2 = "Connection Failed"
        $hashStatus3 = "Connection Failed"
        $hashStatus4 = "Connection Failed"
    }
...
if ($results.Count -eq 0) {
    Write-Host "? No results found. Exiting..." -ForegroundColor Red
    exit
}
...
$HtmlBody = foreach ($result in $results) {
    $c1 = if ($result.CoreIssue_dbb -eq "Identical") { "ok" } else { "fail" }
    $c2 = if ($result.CoreAuth_dbb -eq "Identical") { "ok" } else { "fail" }
    $c3 = if ($result.CoreIssue_error -eq "Identical") { "ok" } else { "fail" }
    $c4 = if ($result.CoreAuth_error -eq "Identical") { "ok" } else { "fail" }

    "<tr>
        <td>$($result.Server)</td>
        <td class='$c1'>$($result.CoreIssue_dbb)</td>
        <td class='$c2'>$($result.CoreAuth_dbb)</td>
        <td class='$c3'>$($result.CoreIssue_error)</td>
        <td class='$c4'>$($result.CoreAuth_error)</td>
    </tr>"
}
