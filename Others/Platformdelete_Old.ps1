...
#-------------All file delete inside folder-----------

     Get-ChildItem -Path "D:\CC_Runtime\*"  | Remove-Item -Recurse -Force 

#-------------Specific  file delete inside folder-----------

    #Remove-Item -Path C:\corecard_services\shmem.bin -Force

#-------------Folder delete-----------

    #Remove-Item -Path "E:\Folders" -Recurse -Force

    #----Deleted files  inside folder and print deleted file ---------

    #Get-ChildItem -Path "E:\Folders\*" -Recurse -Force | ForEach-Object { try { Remove-Item -Path $_.FullName -Recurse -Force -ErrorAction Stop; Write-Host "Deleted: $($_.FullName)" -ForegroundColor Green } catch { Write-Host "Failed to delete: $($_.FullName). Error: $($_.Exception.Message)" -ForegroundColor Red } }
