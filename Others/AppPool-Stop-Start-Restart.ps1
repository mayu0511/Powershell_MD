...
# App Pool Stop -Restart  | DEVELOPED BY:: Mahendra Dwivedi
...
$Results = Invoke-Command -ComputerName $ServerList -SessionOption $option -ErrorAction Continue -ArgumentList $Operation -ScriptBlock {
    ...
    $AppPoolName = "NetworkManagementAPI"
...
$ReportFile = Join-Path $ReportFolder "AppPool-Status_$DateTime.html"

$HtmlHeader = @"
<html>
<head>
<title>AppPool Status Report</title>
<style>
body {
    font-family: Arial;
    font-size: 10pt;
...
$Results |
    ...
    ConvertTo-Html `
    -Property ServerName,AppPoolName,Status `
    -Head $HtmlHeader `
