...
# Version 3.0 | Date:: 07-April-2026 | Update By Mahendrta Dwivedi
...
foreach ($ServerType in $ServerTypes) {
    ...
    $ServerList = (aws ec2 describe-instances --query "Reservations[*].Instances[*].{AvailabilityZone:Placement.AvailabilityZone,Name:Tags[?Key=='Name']|[0].Value}" `
        --filters "Name=instance-state-name,Values=running" "Name=tag:Name,Values='*$ServerType$ShortRegion$EnvironmentName$EnvironmentStack*'" `
...
$Outputreport = $Outputreport -replace '<tr>(.*?)<td>(.*?)gmsa-app-upg\$(.*?)</td>(.*?)<td>ZDT</td>', '<tr>$1<td bgcolor="lightsalmon">$2gmsa-app-upg$$3</td>$4<td bgcolor="lightsalmon">ZDT</td>'
