...
if ($zipMainChoice -eq "yes") {
    ...
    # 7-ZIP CONFIG
    # ========================
    $sevenZipPath = "D:\CC_Scripts\Powershell_MD\Others\7-Zip\7z.exe"
    ...
    try {
        ...
        # ?? Important: use * to avoid double folder issue
        & $sevenZipPath a -tzip $mainZip "$basePath\*" -mx=5 | Out-Null

        Write-Host "ZIP created successfully: $mainZip" -ForegroundColor Green
    }
    catch {
