...
# Version 5.4 | ORIGINAL STYLE (Empty Folder Fix Added, No Minimization)
############################################################################################################
...
function Invoke-PackageDownloadAndDeploy {
    ...
    # ========================
# Task-28.1 DELETE ZIP FILES (MonitoringScript)
# ========================

$monitoringPath = Join-Path $dbbPath "MonitoringScript"

if (Test-Path $monitoringPath) {

    $zipFiles = Get-ChildItem $monitoringPath -Filter *.zip -File -ErrorAction SilentlyContinue

    if ($zipFiles.Count -gt 0) {

        Log "Found $($zipFiles.Count) zip file(s) in MonitoringScript"

        $userChoice = Read-Host "Do you want to delete these zip files? (y/n)"

        if ($userChoice.Trim().ToLower() -eq "y") {
            ...
            try {
                ...
                Log "Failed to delete some zip files"
            }

        } else {
            Log "Skipping deletion and moving to next step..."
        }
    ...
    Log "MonitoringScript folder not found"
}

# Next Task continues automatically here
...
if (-not $taskFiles) {
    ...
    foreach ($file in $taskFiles) {
        ...
        try {
            ...
            # Extract UserId
            if ($xmlContent -match '<UserId>(.*?)</UserId>') {
                $xmlUser = $matches[1]
            }
            else {
                Log "FAILED: $taskName - No UserId found in XML"
                continue
            }
            ...
            if ($xmlUser -like "*$") {
                ...
                # Ensure correct LogonType
                $xmlContent = $xmlContent -replace '<LogonType>.*?</LogonType>', "<LogonType>ServiceAccount</LogonType>"
                ...
                Log "IMPORTED: $taskName (gMSA: $xmlUser)"
            }
            ...
            # ========================
                ...
                Log "INFO: $taskName uses domain user ($xmlUser)"

                # Ask password once per user
                if (-not $credCache.ContainsKey($xmlUser)) {
                    $securePass = Read-Host "Enter password for $xmlUser" -AsSecureString
                    $credCache[$xmlUser] = $securePass
                }

                $securePass = $credCache[$xmlUser]
                ...
                Log "IMPORTED: $taskName (Domain User: $xmlUser)"
            }
...
foreach ($file in $taskFiles) {
    ...
    try {
        ...
        if ($task) {
            ...
            $runUser = $task.Principal.UserId

            if ($runUser -like "*$") {
                Log "VALID: $taskName (gMSA: $runUser)"
            }
            elseif ($runUser) {
                Log "VALID: $taskName (User: $runUser)"
            }
            else {
                Log "CHECK: $taskName (User not set properly)"
            }
        }
        else {
