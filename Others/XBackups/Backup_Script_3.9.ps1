...
# Version 3.9 | ORIGINAL STYLE (Empty Folder Fix Added, No Minimization)
############################################################################################################
...
function Run-Backup {
    ...
    foreach ($sourcePath in $sources) {
        ...
        $itemsToCopy = $itemsToCopy | Where-Object {
            ...
            if ($_.FullName -like "D:\DBBSetup\MonitoringScript\PlatStatementAudit\*" -and $_.Extension -eq ".txt") { return $false }
...
if ($zipMainChoice -eq "yes") {
    ...
    Write-Host "Zipping main folder..."

    $itemsToZip = Get-ChildItem -Path $basePath -Force | ForEach-Object {
        $_.FullName
    }

    if ($itemsToZip.Count -eq 0) {
        Write-Host "Nothing to zip inside $basePath" -ForegroundColor Yellow
    } else {
        Compress-Archive -Path $itemsToZip -DestinationPath $mainZip -Force
    }

    try {
        [System.IO.Compression.ZipFile]::OpenRead($mainZip).Dispose()
        Write-Host "Main ZIP validated: $mainZip" -ForegroundColor Green
    } catch {
        Write-Host "Main ZIP is invalid!" -ForegroundColor Red
    }
