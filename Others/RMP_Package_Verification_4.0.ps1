######################################################################################################################
# RMP Package Validation | DEVELOPED BY:: Mahendra Dwivedi
# Version 1.3 | RMP Package Validation | Date:: 07-Aug-2025
#======================================================================================================================
...
# --- Close all SMB sessions ---
    ...
    $packagedir = 'E:\Packages\COOKIE\POD2\GSUAT\'
  ...
  # ------------------ Step 2: Confirm Zipping (With Root Folder) ------------------
...
# ------------------ Step 5: List ZIP Contents ------------------
    ...
    # ------------------ Step 7: Zip the Package Folder (Using 7-Zip) ------------------
...
function Compress-With7Zip {
    ...
    $sevenZip = "C:\Program Files\7-Zip\7z.exe"  # Change path if you have a portable version

    if (-not (Test-Path $sevenZip)) {
        Write-Host "❌ 7-Zip not found at: $sevenZip" -ForegroundColor Red
        Log-Step "ERROR: 7-Zip not found at $sevenZip"
        return
    }
    ...
    try {
        ...
        if ($process.ExitCode -eq 0) {
            Write-Host "`n✅ ZIP created successfully: $destinationZip" -ForegroundColor Green
...
if ($confirmZipFinal -match '^[Yy]$') {
    ...
    # ✅ Use 7-Zip instead of Compress-Archive
    Compress-With7Zip -sourceFolder $package -destinationZip $zipFile
...
# ------------------ Step 11: Zip the Main Package Folder (Faster using 7-Zip) ------------------
...
if ($zipConfirm -match '^[Yy]$') {
    if (Test-Path $sourceFolder) {
        ...
        try {
            ...
            $7zipPath = "C:\Program Files\7-Zip\7z.exe"  # Or use bundled version path
            ...
            $zipCommand = "`"$7zipPath`" a -tzip `"$destinationZip`" `"$rootFolderName\*`""
