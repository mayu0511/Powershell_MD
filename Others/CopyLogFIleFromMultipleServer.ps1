...
mkdir D:\Temp1\
#Copy-Item -Path  C:\corecard_services\traces\_dbbScaleService_ -Destination D:\Temp1\$env:computername.log
Copy-Item -Path  C:\corecard_services\traces\* -Destination D:\Temp1\$env:computername
#Read-Host -Prompt "Press Any Ket To Continue"
sleep -Seconds 3

#Compress-Archive -Path D:\Temp1\* -Update -DestinationPath D:\Temp1\$env:computername.zip
#aws s3 cp D:\Temp1\$env:computername.zip s3://corecard-pod2-prod-us-west-2-config-files/Copy/Temp/$env:computername.zip
aws s3 cp D:\Temp1\ s3://corecard-pod2-prod-us-west-2-config-files/Copy/Temp/ --recursive
#Remove-Item -Path D:\Temp1\*.log* -recurse
}
}
aws s3 cp s3://corecard-pod2-prod-us-west-2-config-files/Copy/Temp/ D:\TempLog\ --recursive
aws s3 rm s3://corecard-pod2-prod-us-west-2-config-files/Copy/Temp/ --recursive
#Get-ChildItem 'D:\TempLog' -Filter *.zip | Expand-Archive -DestinationPath 'D:\TempLog' -Force
#Remove-Item -Path D:\TempLog\*.zip* -recurse
