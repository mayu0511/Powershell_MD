...
# Print Table with Conditional Coloring
Write-Host "`nServer Name  DeviceID  TotalSize(GB)  UsedSpace(GB)  FreeSpace(GB)  Usage(%)" -ForegroundColor Cyan
Write-Host "-----------  --------  -------------  -------------  -------------  --------"

foreach ($entry in $AllResults) {
    $usage = $entry.'Usage(%)'
    $color = if ($usage -gt 30) { "Red" } else { "White" }

    Write-Host ($entry.'Server Name'.PadRight(12) + " " + 
                $entry.'DeviceID'.PadRight(8) + " " + 
                $entry.'TotalSize(GB)'.ToString().PadRight(13) + " " + 
                $entry.'UsedSpace(GB)'.ToString().PadRight(13) + " " + 
                $entry.'FreeSpace(GB)'.ToString().PadRight(13) + " " + 
                $entry.'Usage(%)'.ToString().PadRight(8)) -ForegroundColor $color
}
